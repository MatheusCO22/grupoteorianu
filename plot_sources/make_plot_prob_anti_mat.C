...
void make_plot_prob_anti_mat()
{
    string dat_dir = "../dat_files/";
    
    //MATERIA ANTI NEUTRINOS
    //Gerar canvas
    int width = 2;
    auto c1 = new TCanvas("c4", "Probabilidades na materia");
    c1->SetGrid();
    c1->SetLogx();
                            // x1   y1   x2   y2
    auto legend = new TLegend(0.7, 0.9, 0.9, 0.8);

    auto gr_antie = new TGraph((dat_dir+"probability_DUNE_antie_mat.dat").c_str());
    auto gr_antimu = new TGraph((dat_dir+"probability_DUNE_antimu_mat.dat").c_str());
    auto gr_antitau = new TGraph((dat_dir+"probability_DUNE_antitau_mat.dat").c_str());
    ...
    g->SetTitle("Anti Muon oscillation probabilities (matter);Energy (GeV);Oscillation probability");
